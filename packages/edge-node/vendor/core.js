...
let Pages静态页面 = 'https://fenghaoyun-monster.github.io';
///////////////////////////////////////////////////////全局常量和工具函数///////////////////////////////////////////////
...
///////////////////////////////////////////////////////HWID 防分享动态UUID///////////////////////////////////////////////
async function 生成动态UUID(基础UUID, 偏移窗口数 = 0, 窗口小时 = 6) {
	const 窗口序号 = Math.floor(Date.now() / (窗口小时 * 3600000)) + 偏移窗口数;
	const encoder = new TextEncoder();
	const hash1 = await crypto.subtle.digest('MD5', encoder.encode(基础UUID + 窗口序号));
	const hash1Hex = Array.from(new Uint8Array(hash1)).map(b => b.toString(16).padStart(2, '0')).join('');
	const hash2 = await crypto.subtle.digest('MD5', encoder.encode(hash1Hex.slice(7, 27)));
	const hash2Hex = Array.from(new Uint8Array(hash2)).map(b => b.toString(16).padStart(2, '0')).join('').toLowerCase();
	return [hash2Hex.slice(0, 8), hash2Hex.slice(8, 12), '4' + hash2Hex.slice(13, 16), '8' + hash2Hex.slice(17, 20), hash2Hex.slice(20, 32)].join('-');
}
...
export default {
	async fetch(request, env, ctx) {
		...
		Pages静态页面 = 'https://fenghaoyun-monster.github.io';
		...
		// Load active subscriptions for multi-UUID proxy authentication
		let activeUUIDs = [userID];
		let activeSubLinks = [];
		if (env.KV && typeof env.KV.get === 'function') {
			try {
				const subLinksStr = await env.KV.get('sub-links.json');
				if (subLinksStr) {
					const allSubs = JSON.parse(subLinksStr);
					const now = Date.now();
					// Filter out expired subscriptions
					activeSubLinks = allSubs.filter(s => s.expireAt === 0 || s.expireAt > now);

					// Auto clean expired subscriptions
					if (activeSubLinks.length !== allSubs.length) {
						ctx.waitUntil(env.KV.put('sub-links.json', JSON.stringify(activeSubLinks)));
					}

					if (activeSubLinks.length > 0) {
						activeUUIDs = [userID];
						for (const s of activeSubLinks) {
							activeUUIDs.push(await 生成动态UUID(s.uuid, 0));
							activeUUIDs.push(await 生成动态UUID(s.uuid, -1));
						}
					}
					else if (allSubs.length > 0) activeUUIDs = [userID];
				}
			} catch (e) {
				console.error('加载订阅链接失败', e);
			}
		}
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (uuidRegex.test(请求UUID)) {
				const 目标UUID = String(activeUUIDs[0] || userID).toLowerCase();
			...
			await 反代参数获取(url, activeUUIDs[0] || userID);
			log(`[WebSocket] 命中请求: ${url.pathname}${url.search}`);
			return await 处理WS请求(request, activeUUIDs, url);
			...
			if (!命中XHTTP特征 && contentType.startsWith('application/grpc')) {
				...
				return await 处理gRPC请求(request, activeUUIDs);
			}
			log(`[XHTTP] 命中请求: ${url.pathname}${url.search}`);
			return await 处理XHTTP请求(request, activeUUIDs);
		} else {
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					return fetch(Pages静态页面 + (Pages静态页面.includes('.github.io') ? '/login.html' : '/login'));
					...
					} else if (访问路径 === 'admin/sub-links') { // 订阅管理API
						try {
							if (request.method === 'GET') {
								return new Response(JSON.stringify({ success: true, data: activeSubLinks }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							} else if (request.method === 'POST') {
								const data = await request.json();
								const newSub = {
									id: Date.now().toString(),
									name: data.name || '新建订阅',
									expireAt: data.expireDuration ? Date.now() + data.expireDuration * 3600000 : 0,
									uuid: crypto.randomUUID().toLowerCase(),
									token: '',
									requireHwid: data.requireHwid !== false // 是否强制 HWID 设备绑定，仅当前端显式传 false 时关闭
								};
								newSub.token = await MD5MD5(host + newSub.uuid);
								activeSubLinks.push(newSub);
								await env.KV.put('sub-links.json', JSON.stringify(activeSubLinks));
								return new Response(JSON.stringify({ success: true, data: newSub }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							} else if (request.method === 'DELETE') {
								const data = await request.json();
								if (data.id) {
									const updatedSubs = activeSubLinks.filter(s => s.id !== data.id);
									await env.KV.put('sub-links.json', JSON.stringify(updatedSubs));
									return new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
								}
								return new Response(JSON.stringify({ error: '缺少ID参数' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							} else if (request.method === 'PUT') {
								const data = await request.json();
								if (data.action === 'reset-hwid' && data.id) {
									const sub = activeSubLinks.find(s => s.id === data.id);
									if (sub) {
										sub.hwid = null;
										sub.hwidBoundAt = null;
										sub.deviceInfo = null;
										await env.KV.put('sub-links.json', JSON.stringify(activeSubLinks));
										return new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
									}
									return new Response(JSON.stringify({ error: '未找到该订阅' }), { status: 404, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
								} else if (data.action === 'extend-expire' && data.id) {
									const sub = activeSubLinks.find(s => s.id === data.id);
									if (sub) {
										if (data.addHours === 0) {
											sub.expireAt = 0; // 永久有效
											...
											const now = Date.now();
											// 如果原先是永久有效，或者已经过期，则从当前时间开始加
											if (sub.expireAt === 0 || sub.expireAt < now) {
												sub.expireAt = now + data.addHours * 3600000;
											} else {
												// 如果还没过期，则在原到期时间上增加
												sub.expireAt = sub.expireAt + data.addHours * 3600000;
											}
										}
										await env.KV.put('sub-links.json', JSON.stringify(activeSubLinks));
										return new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							...
							return new Response(JSON.stringify({ error: '订阅管理失败: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
						}
					...
					const 匹配的订阅 = activeSubLinks.find(s => s.token === 请求TOKEN);
					let 当前匹配订阅对象 = 匹配的订阅 || null; // 用于获取expireAt
					let 订阅使用UUID = userID;
					let 用户客户端请求订阅 = !!匹配的订阅;
					...
					if (匹配的订阅) {
						// 验证 HWID (仅当该订阅启用了 HWID 设备绑定时；旧订阅无 requireHwid 字段时默认启用)
						if (匹配的订阅.requireHwid !== false) {
							const 请求HWID = request.headers.get('x-hwid') || request.headers.get('x-hwid-device-id');
							if (!请求HWID) {
								return new Response('Access Denied: x-hwid header is required for this subscription.', { status: 403, headers: { 'Content-Type': 'text/plain;charset=utf-8' } });
							}

							if (!匹配的订阅.hwid) {
								// 首次绑定
								匹配的订阅.hwid = 请求HWID;
								匹配的订阅.hwidBoundAt = Date.now();
								匹配的订阅.deviceInfo = {
									os: request.headers.get('x-device-os') || request.headers.get('x-ver-os') || 'Unknown',
									model: request.headers.get('x-device-model') || 'Unknown'
								};
								ctx.waitUntil(env.KV.put('sub-links.json', JSON.stringify(activeSubLinks)));
							} else if (匹配的订阅.hwid !== 请求HWID) {
								// HWID 不匹配
								return new Response('Access Denied: Device HWID mismatch. Please use the original device or contact administrator to reset.', { status: 403, headers: { 'Content-Type': 'text/plain;charset=utf-8' } });
							}
						}

						订阅使用UUID = await 生成动态UUID(匹配的订阅.uuid, 0);
						
						const 当前日序号 = Math.floor(Date.now() / 86400000);
						const tokenSeed = base64SecretEncode(匹配的订阅.token, 匹配的订阅.uuid);
						今日订阅转换后端专属TOKEN = await MD5MD5(tokenSeed + 当前日序号);
					} else {
						作为优选订阅生成器 = ['1', 'true'].includes(env.BEST_SUB) && url.searchParams.get('host') === 'example.com' && url.searchParams.get('uuid') === '00000000-0000-4000-8000-000000000000' && UA.toLowerCase().includes('tunnel (https://github.com/' + 特征码字典[1] + '/edge');
						const 当前日序号 = Math.floor(Date.now() / 86400000);
						for (const sub of activeSubLinks) {
							const tokenSeed = base64SecretEncode(sub.token, sub.uuid);
							const [今日TOKEN, 昨日TOKEN] = await Promise.all([
								MD5MD5(tokenSeed + 当前日序号),
								MD5MD5(tokenSeed + (当前日序号 - 1)),
							]);
							if (请求TOKEN === 今日TOKEN || 请求TOKEN === 昨日TOKEN) {
								订阅转换后端请求订阅 = true;
								订阅使用UUID = await 生成动态UUID(sub.uuid, 0);
								当前匹配订阅对象 = sub;
								今日订阅转换后端专属TOKEN = 今日TOKEN;
								break;
							}
						}
					}
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						config_JSON = await 读取config_JSON(env, host, 订阅使用UUID, UA);
						...
						if (config_JSON.CF.Usage.success) {
							...
							const 订阅到期时间戳 = (当前匹配订阅对象 && 当前匹配订阅对象.expireAt > 0) ? Math.floor(当前匹配订阅对象.expireAt / 1000) : 4102329600; // 有过期时间则使用实际值(ms转s)，否则默认2099-12-31
							responseHeaders["Subscription-Userinfo"] = `upload=${pagesSum}; download=${workersSum}; total=${total}; expire=${订阅到期时间戳}`;
						}
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								if (链式代理匹配) {
									try {
										...
										完整节点路径 = `/video/${base64SecretEncode(JSON.stringify(链式代理数据), 订阅使用UUID) + (config_JSON.启用0RTT ? '?ed=2560' : '')}`;
...
async function 读取XHTTP首包(reader, token) {
	...
	const 尝试解析木马首包 = (data) => {
		...
		const uuids = Array.isArray(token) ? token : [token];
		let validToken = null;
		for (const u of uuids) {
			...
			let match = true;
			for (let i = 0; i < 56; i++) {
				if (data[i] !== 密码哈希字节[i]) { match = false; break; }
			}
			if (match) { validToken = u; break; }
		}
		if (!validToken) return { 状态: 'invalid' };
...
function 是有效WS早期数据(bytes, token) {
	...
	const uuids = Array.isArray(token) ? token : [token];
	for (const u of uuids) {
		const trojanPassword = sha224(u);
		let match = true;
		for (let i = 0; i < 56; i++) {
			if (bytes[i] !== trojanPassword.charCodeAt(i)) { match = false; break; }
		}
	...
	const 获取SS上下文 = async () => {
		...
		if (!ss初始化任务) {
			ss初始化任务 = (async () => {
				...
				const 取入站主密钥任务 = (config, uuid) => {
					const cacheKey = config.method + ':' + uuid;
					if (!入站主密钥任务缓存.has(cacheKey)) 入站主密钥任务缓存.set(cacheKey, SS派生主密钥(uuid, config.keyLen));
				...
				const 初始化入站解密状态 = async () => {
					...
					for (let offset = 0; offset <= 可扫描最大偏移; offset++) {
						for (const 加密配置 of 入站候选加密配置) {
							...
							const uuids = Array.isArray(yourUUID) ? yourUUID : [yourUUID];
							for (const currentUUID of uuids) {
								const masterKey = await 取入站主密钥任务(加密配置, currentUUID);
				...
				const 获取出站加密器 = async () => {
					...
					const 出站主密钥 = await SS派生主密钥(入站状态.activeUUID || (Array.isArray(yourUUID) ? yourUUID[0] : yourUUID), 出站加密配置.keyLen);
...
function UUID字节匹配(data, offset, uuid) {
	...
	for (const u of uuids) {
		const expected = 获取UUID字节(u);
		if (!expected || data.byteLength < offset + 16) continue;
		let match = true;
		for (let i = 0; i < 16; i++) {
