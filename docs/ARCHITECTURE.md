...
| 维度 | 你的选择 | 对架构的含义 |
|---|---|---|
| Dispatch 落地 | **多账号批量部署** | 数据面 = 跨多个 CF 账号、由 CI 批量部署的大量独立 Worker；免费、抗封号、可无限横向扩容。**不**用付费的 Workers for Platforms。 |
...
一句话形态：**你在中心后台建用户、分配节点组、下发订阅；成百上千个边缘节点分散在多个 CF 账号里跑，
统一从控制面拉取「谁有效」的名单来鉴权；某个节点被封了，编排层自动重部署、订阅自动愈合。**
...
- **GrainTCPV1**：用 D1 做了后端、支持 Snippets——有「后台」的雏形，但没有多账号批量分发、没有自愈轮换。
...
- **分流策略下发**：解锁域名清单由控制面按套餐下发（而非硬编码），实现「可切换分流」。
- **签名校验**：与控制面之间所有拉取/上报用 HMAC(共享密钥) 防伪造。
- **尽力遥测**：连接计数、粗略流量回传。
...
- **健康探测 + 自愈** `healthcheck.yml`（定时）：探活所有节点，标记/剔除被封节点，必要时换域名/换账号重部署。
...
| 层 | 选型 | 说明 |
|---|---|---|
| 边缘节点运行时 | Cloudflare Workers | 多账号矩阵部署，免费额度足够横向扩 |
...
| 编排/CI | GitHub Actions + wrangler + CloudflareSpeedTest | 多账号矩阵、优选、自愈 |
...
- **CF ToS / 封号**：在 Workers 上跑代理、尤其规模化分发，违反 Cloudflare 服务条款，账号可能被封。**这正是本架构用「多账号 + 自愈轮换」的原因**——把封号当作常态来设计，而非事故。请用**独立的、非主力**的 CF 账号，别绑重要业务/付款。
- **无实时精确流量计量**：Workers 不适合做精确按字节计费/限速；本设计的遥测是「尽力而为」（连接数、粗略字节），硬限额能力弱。若二期要严格计量，需把「计量出口」放到你可控的落地机上做。
- **免费额度**：每账号 Workers 免费额度约 10 万请求/天、CPU 时间受限；多账号矩阵正是为摊薄这个限制。
